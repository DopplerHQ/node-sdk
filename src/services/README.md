...
(async () => {
  const input = {
    access: 'read',
    config: 'CONFIG_NAME',
    expire_at: '1928-03-09T03:41:46.0Z',
    name: 'TOKEN_NAME',
    project: 'PROJECT_NAME',
  };
...
(async () => {
  const input = {
    environments: ['eiusmod aliqua Ut Excepteur sed', 'dolore tempor sunt dolore'],
    role: 'role',
    slug: 'slug',
...
(async () => {
  const input = {
    environments: ['dolor incididunt minim', 'non adipisicing eu ullamco'],
    role: 'role',
  };
...
(async () => {
  const input = {
    ...
    workplace_role: {
      ...
      permissions: ['magna reprehenderit ipsum cillum in', 'Ut'],
    },
  };
...
(async () => {
  const input = {
    name: 'name',
    workplace_role: { identifier: 'identifier', permissions: ['anim aliqua', 'quis'] },
  };
