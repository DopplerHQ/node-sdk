...
(async () => {
  const input = {
    access: 'read',
    config: 'CONFIG_NAME',
    expire_at: '1902-04-13T09:58:51.0Z',
    name: 'TOKEN_NAME',
    project: 'PROJECT_NAME',
  };
...
(async () => {
  const input = { environments: ['Ut', 'pariatur'], role: 'role', slug: 'slug', type_: 'group' };
...
(async () => {
  const result = await sdk.projectMembers.get('project', 'invite', 'slug');
  console.log(result);
})();
...
(async () => {
  const input = { environments: ['adipisicing anim', 'tempor velit sunt'], role: 'role' };
...
(async () => {
  const input = {
    ...
    workplace_role: {
      ...
      permissions: ['non in eiusmod ullamco id', 'Duis do tempor quis'],
    },
  };
...
(async () => {
  const input = {
    ...
    workplace_role: {
      identifier: 'identifier',
      permissions: ['et Excepteur ea sed', 'anim elit voluptate est consequat'],
    },
  };
