...
describe('test Users', () => {
  ...
  describe('test get', () => {
    test('test api call', () => {
      const scope = nock('https://api.doppler.com')
        .get('/v3/workplace/users/dolor')
        .reply(200, { data: {} });
      return sdk.users.get('dolor').then((r: any) => expect(r.data).toEqual({}));
    });

    test('test will throw error if required fields missing', () => {
      const scope = nock('https://api.doppler.com')
        .get('/v3/workplace/users/ipsum')
        .reply(200, { data: {} });
    ...
    test('test will throw error on a non-200 response', () => {
      const scope = nock('https://api.doppler.com')
        .get('/v3/workplace/users/nulla')
        .reply(404, { data: {} });
      return expect(async () => await sdk.users.get('nulla')).rejects.toThrow();
    });
  });
});
