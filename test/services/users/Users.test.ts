...
describe('test Users', () => {
  ...
  describe('test list', () => {
    test('test api call', () => {
      const scope = nock('https://api.doppler.com')
        .get('/v3/workplace/users?page=9')
        .reply(200, { data: {} });
      return sdk.users.list({ page: 9 }).then((r: any) => expect(r.data).toEqual({}));
    });
  });
  ...
  describe('test get', () => {
    ...
    test('test will throw error if required fields missing', () => {
      const scope = nock('https://api.doppler.com')
        .get('/v3/workplace/users/corrupti')
        .reply(200, { data: {} });
    ...
    test('test will throw error on a non-200 response', () => {
      const scope = nock('https://api.doppler.com')
        .get('/v3/workplace/users/nisi')
        .reply(404, { data: {} });
      return expect(async () => await sdk.users.get('nisi')).rejects.toThrow();
    });
  });
});
