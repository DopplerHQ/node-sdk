  ...
  describe('test list', () => {
    test('test api call', () => {
      const scope = nock('https://api.doppler.com')
        .get('/v3/configs/config/tokens?project=architecto&config=quis')
        .reply(200, { data: {} });
      return sdk.serviceTokens
        .list('architecto', 'quis')
        .then((r: any) => expect(r.data).toEqual({}));
    });

    test('test will throw error if required fields missing', () => {
      const scope = nock('https://api.doppler.com')
        .get('/v3/configs/config/tokens?project=cupiditate&config=repudiandae')
        .reply(200, { data: {} });
    ...
    test('test will throw error on a non-200 response', () => {
      const scope = nock('https://api.doppler.com')
        .get('/v3/configs/config/tokens?project=saepe&config=nesciunt')
        .reply(404, { data: {} });
      return expect(
        async () => await sdk.serviceTokens.list('saepe', 'nesciunt'),
      ).rejects.toThrow();
    });
  });
