...
describe('test Projects', () => {
  ...
  describe('test list', () => {
    test('test api call', () => {
      const scope = nock('https://api.doppler.com')
        .get('/v3/projects?page=8&per_page=3')
        .reply(200, { data: {} });
      return sdk.projects
        .list({ page: 8, perPage: 3 })
  ...
  describe('test get', () => {
    test('test api call', () => {
      const scope = nock('https://api.doppler.com')
        .get('/v3/projects/project?project=reprehenderit')
        .reply(200, { data: {} });
      return sdk.projects.get('reprehenderit').then((r: any) => expect(r.data).toEqual({}));
    });

    test('test will throw error if required fields missing', () => {
      const scope = nock('https://api.doppler.com')
        .get('/v3/projects/project?project=facilis')
        .reply(200, { data: {} });
    ...
    test('test will throw error on a non-200 response', () => {
      const scope = nock('https://api.doppler.com')
        ...
        .reply(404, { data: {} });
      return expect(async () => await sdk.projects.get('vel')).rejects.toThrow();
    });
  });
